...
export class Relation {
  ...
  getRelatedCells(cellId: string): Cell[] {
    return (this.relatedLeftCells[cellId] || []).concat(
      this.relatedRightCells[cellId] || []
    );
  }
  ...
  updateRelation(): void {
    ...
    if (this.matchType === RelationMatchType.Fuzzy) {
      ...
      for (const cellLeft of this.notebookLeft.cellList) {
        const cellRightList = this.notebookRight
          .getCellsByMeme(cellLeft.meme)
      ...
      for (const cellLeft of this.notebookLeft.cellList.filter(
        cell => !this.relatedRightCells[cell.id].length
      )) {
